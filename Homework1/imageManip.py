...
def dim_image(image):
    ...
    ### YOUR CODE HERE
    out = np.dot(0.5, np.square(image))
    ### END YOUR CODE
...
def rotate2d(point, theta):
    ...
    ## YOUR CODE HERE
    return np.dot([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]], point)
    ### END YOUR CODE
...
def rotate_image(input_image, theta):
    ...
    ## YOUR CODE HERE
    rotate_center_cordinate = np.array([int(input_rows/2), int(input_cols/2)])
    translate_matrix = np.identity(3)
    translate_matrix_inverse = np.identity(3)

    translate_matrix[0:2, 2] = -rotate_center_cordinate[0:2]
    translate_matrix_inverse[0:2, 2] = rotate_center_cordinate[0:2]
        
    for row in range(input_rows):
        for col in range(input_cols):
            x, y = np.dot(translate_matrix, np.array([row, col, 1]))[0:2]
            x, y = rotate2d(np.array([x, y]), -theta) # 
            x, y = np.dot(translate_matrix_inverse, np.array([x, y, 1]))[0:2]
            if x < 0 or x >= input_rows or y < 0 or y >= input_cols:
                continue
            output_image[int(x), int(y), :] = input_image[row, col, :]
